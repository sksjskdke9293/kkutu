...
exports.submit = function(client, text, data){
	...
	if(!forceDev && !isChainable(text, my.mode, my.game.char, my.game.subChar)) return client.chat(text);
	...
	function onDB($doc){
		...
		function preApproved(){
			...
			if(forceDev) approved();
			else if(firstMove || my.opts.manner) getAuto.call(my, preChar, preSubChar, 1).then(function(w){
				if(w) approved();
				else{
					my.game.loading = false;
	...
	if(forceDev){
		onDB({ mean: '[DEV] 자동 생성 테스트 단어', theme: 'DEV', type: 'DEV', flag: 0, hit: 0 });
	}else DB.kkutu[l].findOne([ '_id', text ],
