...
DB.ready = function(){
	...
	Server.listen(Number(process.env.KKUTU_WEB_PORT || 3000));
	if(Const.IS_SECURED) {
...
Server.get("/", function(req, res){
	...
	function onFinish($doc){
		...
		page(req, res, Const.MAIN_PORTS[server] ? "kkutu" : "portal", {
			'_page': "kkutu",
			'_id': id,
			'PORT': process.env.RENDER ? null : Const.MAIN_PORTS[server],
			'HOST': process.env.RENDER_EXTERNAL_HOSTNAME || req.hostname,
			'PROTOCOL': process.env.RENDER ? 'wss' : (Const.IS_SECURED ? 'wss' : 'ws'),
			'TEST': req.query.test,
